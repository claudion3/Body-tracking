...
const Profile: React.FC = () => {
    ...
    const fetchProfile = useCallback(async () => {
        ...
        try {
            ...
            setProfile({
                ...
                goalWeight: response.data.goalWeight ? String(response.data.goalWeight) : '',
    ...
    const validateForm = (profileToValidate: UserProfile): boolean => {
        ...
        if (profileToValidate.age && (Number(profileToValidate.age) < 1 || Number(profileToValidate.age) > 120)) {
            errors.age = 'Age must be between 1 and 120';
        }

        if (profileToValidate.height && (Number(profileToValidate.height) < 50 || Number(profileToValidate.height) > 250)) {
            errors.height = 'Height must be between 50cm and 250cm';
        }

        if (profileToValidate.weight && (Number(profileToValidate.weight) < 20 || Number(profileToValidate.weight) > 300)) {
            errors.weight = 'Weight must be between 20kg and 300kg';
        }

        if (profileToValidate.goalWeight && (Number(profileToValidate.goalWeight) < 20 || Number(profileToValidate.goalWeight) > 300)) {
            errors.goalWeight = 'Goal weight must be between 20kg and 300kg';
        }

        if (profileToValidate.weight && profileToValidate.goalWeight && Number(profileToValidate.weight) < Number(profileToValidate.goalWeight)) {
    ...
    const openModal = useCallback((type: 'edit' | 'password' | 'delete') => {
        setModalType(type);
        if (type === 'edit') {
